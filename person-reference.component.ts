...
import { PersonReference} from '../api/user';  
...
export class PersonReferenceComponent implements OnInit {
  ...
  data: any = [];
  ngOnInit() {
    this._http.get<PersonReference[]>('http://localhost:4600/getPersonRef').subscribe(
      data => {
        this.data = data;
  ...
  onDeleteConfirm(event) {
    this._http.post<any>('http://localhost:4600/deletePersonRef', event.data).subscribe(
      res => {
        console.log(res);
  ...
  onCreateConfirm(event) {
    this._http.post<PersonReference>('http://localhost:4600/savePersonRef', event.newData).subscribe(
      res => {
        console.log(res);
  ...
  onSaveConfirm(event) {
    this._http.post<PersonReference>('http://localhost:4600/updatePersonRef', event.newData).subscribe(
      res => {
        console.log(res);
        event.confirm.resolve(event.newData);
      },
